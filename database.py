...
def insert_student(stu):
    with conn:
        c.execute("INSERT INTO students values(:fingerprint_id, :roll_no, :name)", 
                  {'fingerprint_id': stu.fingerprint_id, 'roll_no': stu.roll_no, 'name': stu.name})
...
def get_stu_by_fingerid(fingerprint_id):
    c.execute("SELECT * FROM students WHERE fingerprint_id=:fingerprint_id", 
